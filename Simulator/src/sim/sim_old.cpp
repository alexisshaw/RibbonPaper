...
ds_Register ds_create_register(int nq_L, double err_L, double sigma_L)
{
    ...
    reg.state = std::vector<ds_Complex>(reg.nc, {0.0,0.0});
    std::fill(reg.state.begin(), reg.state.end(), ds_Complex({0.0,0.0}));
    reg.steps = std::vector<int>(reg.nq, 0);
    std::fill(reg.steps.begin(), reg.steps.end(), 0);
    return reg;
}

std::string ds_string(ds_Register& reg) {
    std::stringstream outbuf;
    for (int i=0; i<reg.nc; i++)
        if (reg.state[i].x !=0 || reg.state[i].y !=0 )
            outbuf << i << " (" << reg.state[i].x << ", " << reg.state[i].y << ")" << '\n';
    return outbuf.str();
}
...
double ds_inner_product(ds_Register& reg1,const ds_Register& reg2)
{
   int i;
   ds_Complex temp;
...
{
    double ds_root2_2 = 1.0 / sqrt(2.0);
   int i, j, k;
   ds_Complex z1, z2;
...
double ds_set_measure_parity(ds_Register& reg, int nq2m, int *lq2m){
    return 0;
}
