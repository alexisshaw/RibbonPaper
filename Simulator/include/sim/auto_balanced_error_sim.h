...
#ifndef STEANE_SIM_AUTO_BALANCED_ERROR_SIM_H
#define STEANE_SIM_AUTO_BALANCED_ERROR_SIM_H
    ...
    void global_update(){
        ...
        for (size_t q=0; q<steps.size(); q++)
            while (steps[q] < max_steps)
                lerr(static_cast<int>(q));
    }

    void lerr(int q){
        if((*rand_gen)() < p) {
            RandomValue rn = (*rand_gen)();
            baseSim.inject_single_pauli_error(q, single_qubit_pauli(rn & 3));
        }
        steps[q] ++;
    }
...
public:
    ...
    AutoBalancedErrorSim(const BaseSim& b, std::shared_ptr<R> rand_gen_in, double p_in = 0.0):
            baseSim{b}, rand_gen{rand_gen_in}, p(static_cast<RandomValue>((p_in * 4.0L)/3.0L*static_cast<double>((R::max() - R::min()))) + R::min()){}

    AutoBalancedErrorSim(BaseSim&& b, std::shared_ptr<R> rand_gen_in, double p_in = 0.0):
            baseSim{std::move(b)}, rand_gen{rand_gen_in}, p(static_cast<RandomValue>((p_in * 4.0L)/3.0L*static_cast<double>((R::max() - R::min()))) + R::min()){}
    ...
    auto measure_all_set(auto qubits, auto required){
        boost::container::small_vector<measurement_state, 20> retval{};
        // global_update();
        // for (auto qubit: qubits){
        //     retval.push_back(measure(qubit));
        // }
        return baseSim.measure_all_set(qubits, required);
    }
    ...
    //! elimination.
    inline bool operator==(AutoBalancedErrorSim<BaseSim, R> &other){
        return baseSim == other.baseSim;
    }

    //! gets the inner product of two states if defined.
    inline double getInnerProduct(AutoBalancedErrorSim<BaseSim, R> &other){
