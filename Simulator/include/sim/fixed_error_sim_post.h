...
#ifndef STEANE_SIM_FIXED_ERROR_SIM_POST_H
#define STEANE_SIM_FIXED_ERROR_SIM_POST_H
...
namespace CodeTools {
    class FixedErrorSimPostBase{

    };

    template<typename BaseSim>
    class FixedErrorSimPost {
    public:
        BaseSim baseSim;
    private:
    ...
    public:
        ...
        explicit FixedErrorSimPost(const BaseSim &b) : baseSim{b} {};

        FixedErrorSimPost(const BaseSim &b, single_qubit_pauli p, int target) :
                baseSim{b}, single_error(p), single_count_target(target) {};

        FixedErrorSimPost(const BaseSim &b, two_qubit_pauli p, int target) :
        ...
        auto measure_all(T qubits){
            ...
            auto retval = baseSim.measure_all(qubits);
            for (auto qubit: qubits){
                maybe_measure_error_post(qubit);
            }
            return retval;
        ...
        auto measure_reset_all(T qubits){
            ...
            auto retval = baseSim.measure_reset_all(qubits);
            for (auto qubit: qubits){
                maybe_measure_error_post(qubit);
            }
            return retval;
        ...
        //! elimination.
        inline bool operator==(FixedErrorSimPost <BaseSim> &other) {
